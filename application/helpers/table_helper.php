...
function get_person_data_row($person,$controller)
{
	...
	$table_data_row.='<td width="20%">'.character_limiter($person->first_name,13).'</td>';
         if($person->email != ''){
            $table_data_row.='<td width="30%">'.anchor($controller_name."/sand_mail/?mail_address=".$person->email,character_limiter($person->email,22)).'</td>';
         }else{
              $table_data_row.='<td width="17%">'.character_limiter($person->email,13).'</td>';
         }
...
function get_supplier_data_row($supplier,$controller)
{
	...
	$table_data_row.='<td width="17%">'.character_limiter($supplier->first_name,13).'</td>';
        if($supplier->email != ''){
            $table_data_row.='<td width="22%">'.anchor($controller_name."/sand_mail/?mail_address=".$supplier->email,character_limiter($supplier->email,22)).'</td>';
        }else{
            $table_data_row.='<td width="17%">'.character_limiter($supplier->email,13).'</td>';
        }
...
*/
function get_branch_manage_table($branch,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';
	
	$headers = array('<input type="checkbox" id="select_all" />',
	$CI->lang->line('branch_code'),
	$CI->lang->line('branch_name'),
	$CI->lang->line('branch_address'),
	$CI->lang->line('manager_name'),
	$CI->lang->line('common_phone_number'),
	'&nbsp');
	...
	$table.='</tr></thead><tbody>';
	$table.=get_branch_manage_table_data_rows($branch,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
*/
function get_branch_manage_table_data_rows($branch,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	
	foreach($branch->result() as $branches)
	{
		$table_data_rows.=get_branch_data_row($branches,$controller);
	}
	
	if($branch->num_rows()==0)
	{
		$table_data_rows.="<tr><td colspan='7'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('common_no_branch_to_display')."</div></tr></tr>";
	}
...
function get_branch_data_row($branch,$controller)
{
	...
	$table_data_row='<tr>';
	$table_data_row.="<td width='5%'><input type='checkbox' id='person_$branch->branch_id' value='".$branch->branch_id."'/></td>";
	$table_data_row.='<td width="17%">'.character_limiter($branch->branch_name,13).'</td>';
	$table_data_row.='<td width="17%">'.character_limiter($branch->branch_code,13).'</td>';
	$table_data_row.='<td width="17%">'.character_limiter($branch->branch_address,13).'</td>';
	$table_data_row.='<td width="22%">'.character_limiter($branch->manager_name,13).'</td>';
	$table_data_row.='<td width="17%">'.character_limiter($branch->branch_phone,13).'</td>';		
...
function get_items_manage_table($items,$controller)
{
	...
	{
		$branch_name = $CI->lang->line('branch_name');
	} */
	$table='<table class="tablesorter" id="sortable_table">';
...
function get_item_data_row($item,$controller)
{
	...
	if($CI->session->userdata('branch_id') == 0)
	{
		$up = anchor($controller_name."/view/$item->item_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_update'))).
			'&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/inventory/$item->item_id/width:$width", $CI->lang->line('common_inv'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_count')));	
	}
	...
	$table_data_row.='<td width="14%">';
	
	if(!empty($item->branch_id)){
		$bran = $CI->db->where('branch_id',$item->branch_id)->get('branch')->row();
		$name = $bran->branch_name;
	}else{
		$name ='';
	}
	
	$table_data_row.= $name.'</td>';	
	$table_data_row.='<td width="14%">'.$item->quantity.'</td>';
	
	//Ramel Inventory Tracking
	$table_data_row.='<td width="15%" align="right">'.$up.'&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/count_details/$item->item_id/width:$width", $CI->lang->line('common_det'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_details_count'))).'</td>';//inventory details	
